...
export default function LeadsPage() {
  ...
  const handleEdit = (lead) => setSelected(lead);

  const handleSave = async () => {
    try {
      setLoading(true);
      const { id, ...payload } = selected;
      const res = await updateLead(selected.id, payload);

      setLeads((prev) =>
        prev.map((l) => (l.id === selected.id ? res.data : l))
      );
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950/80 via-gray-900/80 to-black/80 text-white p-6">
      <Card title="Leads">
        ...
        <div className="text-sm text-gray-400 mb-3">
          {leads.length > 0
            ? `Showing ${(page - 1) * limit + 1} - ${(page - 1) * limit + leads.length} of ${totalCount} leads`
            : "No leads found"}
        </div>
                  ...
                  >
                    ...
                    <td className="p-3">
                      <span
                        ...
                        onClick={async () => {
                          await navigator.clipboard.writeText(l.phoneNo || "");
    ...
    {l.type
      ...
      : "-"}
                    ...
                    <td className="p-3 flex gap-2 justify-center">
                      ...
                      <Button variant="primary" onClick={() => handleEdit(l)}>
